...
int main() {
    ...
    assert(_num_free_blocks() == 31);  // 1 block allocated, 31 free

    // Test 2: Free the block and check if it's marked as free
    sfree(ptr1);
    assert(_num_free_blocks() == 32);  // All blocks should be free again
    assert(_num_free_bytes() == 32 * MAX_BLOCK_SIZE);  // All memory should be free
    assert(_num_allocated_blocks() == 0);

    // Test 3: Allocate multiple blocks and then free them
    void* ptr2 = smalloc(50);
    void* ptr3 = smalloc(150);
    assert(ptr2 != nullptr && ptr3 != nullptr);
    assert(_num_allocated_blocks() == 2);
    assert(_num_free_blocks() == 30);

    sfree(ptr2);
    sfree(ptr3);
    assert(_num_free_blocks() == 32);
    assert(_num_free_bytes() == 32 * MAX_BLOCK_SIZE);

    // Test 4: Allocate a large block that requires mmap
    void* ptr4 = smalloc(MAX_BLOCK_SIZE + 1);
    assert(ptr4 != nullptr);
    assert(_num_allocated_blocks() == 1);
    assert(_num_free_blocks() == 32);  // Only the mmap block is allocated, others are untouched

    sfree(ptr4);
    assert(_num_allocated_blocks() == 0);

    // Test 5: Reallocate a block to a larger size
    void* ptr5 = smalloc(200);
    void* ptr6 = srealloc(ptr5, 400);
    assert(ptr6 != nullptr);
    assert(_num_allocated_blocks() == 1);
    assert(_num_allocated_bytes() == 400);
    assert(_num_free_blocks() == 31);  // 1 block is allocated

    sfree(ptr6);
    assert(_num_allocated_blocks() == 0);
    assert(_num_free_blocks() == 32);

    // Test 6: Reallocate a block to a smaller size
    void* ptr7 = smalloc(300);
    void* ptr8 = srealloc(ptr7, 100);
    assert(ptr8 == ptr7);  // Should reuse the same block
    assert(_num_allocated_blocks() == 1);
    assert(_num_allocated_bytes() == 100);
    ...
    // Test 7: Allocate and immediately free a large number of small blocks
    void* ptrs[100];
    for (int i = 0; i < 100; ++i) {
        ptrs[i] = smalloc(50);
